...
public class WhitelistCommand {
	...
	private static int executeAdd(CommandContext<CommandSource> ctx) throws CommandSyntaxException {
		...
		if(TextileBackup.config.playerWhitelist.contains(player.getName().getString())) {
			ctx.getSource().sendFeedback(new TranslationTextComponent("Player: %s is already whitelisted.", player.getName()), false);
		}else{
			TextileBackup.config.playerWhitelist.add(player.getName().getString());
	...
	private static int executeRemove(CommandContext<CommandSource> ctx) throws CommandSyntaxException {
		...
		if(!TextileBackup.config.playerWhitelist.contains(player.getName().getString())) {
			ctx.getSource().sendFeedback(new TranslationTextComponent("Player: %s newer was on the whitelist.", player.getName()), false);
		}else{
